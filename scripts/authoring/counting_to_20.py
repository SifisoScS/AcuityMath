...
def markdown(problems):
    ...
    w('## 8. What the review could not fix')
    w('')
    w('**Nothing in the application draws these pictures.** The `visual` object is')
    w('served to the client and handed to `MathManipulatives`, which reads')
    w('`visualData.initialCount`, `.itemType`, `.shapeType` and `.formula` — none')
    w('of which exist in this schema — and which only draws at all when')
    w('`visualType` is set to one of five names. `InfiniteAdaptiveModal` hardcodes')
    w('`visualType: undefined` for every served problem, so every branch is skipped')
    w('and the default card renders, which prints the prompt text back under the')
    w('heading *Math Expression*.')
    w('')
    w('For a picture-choice item that means the child is shown a sentence, the same')
    w('sentence again in monospace, and three buttons reading *the first one*, *the')
    w('middle one*, *the last one*, with nothing drawn to tell them apart. The item')
    w('is not hard; it is unanswerable, and guessing is the only available strategy.')
    w('')
    w('**This is not new and not this batch\'s doing.** All 180 `foundations`')
    w('problems are in the same state and have been since they were imported. But')
    w('it bears directly on what reviewing a picture is worth: every drawing')
    w('decision in §2 and §3 — the ten-and-remainder split, the shape switch, the')
    w('spread card, the shared silhouette — is currently invisible. They are')
    w('correct against the schema and against the corpus\'s conventions, and no')
    w('child has ever seen one.')
    w('')
    w('A renderer for this schema is a piece of work in its own right and belongs')
    w('in the roadmap rather than in an authoring pass.')
    w('')
    return '\n'.join(out) + '\n'
